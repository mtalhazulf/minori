<template>
  <q-page class="q-mx-xl">
    <div class="row"><Heading :title="'Tariffe Minori'" /></div>
...
export default {
  ...
  watch: {
    ...
    minors() {
      this.rows = this.formatMinors(this.minors)
    },
    tariffs() {
      this.rows = this.formatMinors(this.minors)
    }
  ...
  methods: {
    ...
    formatMinors(minors) {
      try {
        return minors.map(m => {
          return {
            ...m,
            price: this.tariffs.find(t => t.children_id === m._id.$oid),
            id: m._id.$oid
          }
        })
      } catch (error) {
    ...
    async getTariffs() {
      ...
      try {
        const response = await api.get('minors/minor/price')
    ...
    async getMinors() {
      const path = '/minors/minor/all'
      try {
        const response = await api.get(path)
        this.minors = response.data
        this.data_loading = false
      } catch (error) {
        this.showNotif('negative', 'Errore nel caricamento dei minori')
        this.data_loading = false
      }
    },

    async addTariff(minorId) {
      const path = '/minors/minor/' + minorId + '/price'
      try {
        await api.post(path, {
    ...
    async deleteTariff(minorId, timestamp) {
      this.popupViewHistory = false
      const path = '/minors/minor/' + minorId + '/price/' + timestamp
      try {
